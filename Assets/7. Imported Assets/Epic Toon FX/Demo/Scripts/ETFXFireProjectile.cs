...
namespace EpicToonFX
{
    public class ETFXFireProjectile : MonoBehaviour
    {
        ...
        private void Update()
        {
            if (Input.GetKeyDown(KeyCode.RightArrow)) nextEffect();

            if (Input.GetKeyDown(KeyCode.D)) nextEffect();

            if (Input.GetKeyDown(KeyCode.A))
                previousEffect();
            else if (Input.GetKeyDown(KeyCode.LeftArrow)) previousEffect();
            ...
            if (Input.GetKeyDown(KeyCode.Mouse0)) //On left mouse down-click
                if (!EventSystem.current.IsPointerOverGameObject()) //Checks if the mouse is not over a UI part
                    if (Physics.Raycast(Camera.main.ScreenPointToRay(Input.mousePosition), out hit,
                            100f)) //Finds the point where you click with the mouse
                    {
                        var projectile = Instantiate(projectiles[currentProjectile], spawnPosition.position,
                            Quaternion.identity); //Spawns the selected projectile
                        projectile.transform
                            .LookAt(hit.point); //Sets the projectiles rotation to look at the point clicked
