...
public class ETFXProjectileScript : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        if (GetComponent<Rigidbody>().velocity.magnitude != 0)
            transform.rotation =
                Quaternion.LookRotation(GetComponent<Rigidbody>()
                    .velocity); // Sets rotation to look at direction of movement
        ...
        var detectionDistance =
            ...
            Time.deltaTime; // Distance of collision detection for this frame
                ...
                detectionDistance)) // Checks if collision will happen
        {
            ...
            var impactP = Instantiate(impactParticle, transform.position,
                Quaternion.FromToRotation(Vector3.up, hit.normal)); // Spawns impact effect

            var trails =
                GetComponentsInChildren<ParticleSystem>(); // Gets a list of particle systems, as we need to detach the trails
