...
{
	const float1 wrapMode = animInfo.w;	
	const float1 isPlayingBackwards = animTimeInfo.w < animTimeInfo.z;
	const float1 animRunTime = lerp(
		(_Time.y - animTimeInfo.z) / (animTimeInfo.w - animTimeInfo.z),
		(_Time.y - animTimeInfo.z) / (animTimeInfo.z - animTimeInfo.w),
		isPlayingBackwards);
	const float1 isNormalizedLooping = animRunTime > 0.9999;// step(0.9999, animRunTime);
	const float1 isLoopingWrapMode = step(2.0, wrapMode);
	const float1 isPingPong = step(4.0, wrapMode);
	const float1 isEvenLoop = step(0.5, floor(animRunTime) % 2);
	const float1 invertPingPongLoop = step(1.5, isPingPong + isEvenLoop);
	const float1 normalizedLoopTime = animRunTime - floor(animRunTime);
	// check wrap mode of animation
	float1 normalizedTime = lerp(lerp(normalizedLoopTime, 1.0, isNormalizedLooping), // time if clamped, 0 -> 1
	...
	const float1 currentFrame = min(normalizedTime * animTimeInfo.y, animTimeInfo.y - 1);
	const float1 vertexCount = animInfo.y;
	const float1 textureSize = animInfo.z;
	const float1 framesPerTexture = floor((textureSize * textureSize) / (vertexCount * 2));
    const float1 textureIndexOffset = floor(currentFrame / framesPerTexture);
    textureIndex = floor(textureIndex + textureIndexOffset + 0.1);
	const float1 frameOffset = floor(currentFrame % framesPerTexture);
    const float1 pixelOffset = floor(vertexCount * 2 * frameOffset + 0.5);
	return pixelOffset;
}

float3 GetUVPos(uint vertexIndex, float1 textureIndex, float1 pixelOffset, float1 textureSize, uint offset)
{
	uint vertexOffset = pixelOffset + (vertexIndex * 2);
	vertexOffset += offset;
	float1 offsetX = floor(vertexOffset / textureSize);
	float1 offsetY = vertexOffset - (offsetX * textureSize);
...
{
	float1 textureIndex = animInfo.x - 1.0;
	const float1 pixelOffset = GetPixelOffset(textureIndex, animInfo, animTimeInfo);
	...
	SamplerState samplerState)
{
	if (animInfo.x >= 1.0)
	{
		...
		if (_Time.y < crossfadeData.y && crossfadeData.y > crossfadeData.x)
		{
			...
			const float1 lerpValue = (_Time.y - crossfadeData.x) / crossfadeData.z;
	...
	if (animInfo.x >= 1.0)
	{
		...
		if (normalData.x != 0.0 || normalData.y != 0.0 || normalData.z != 0.0)
		{
			...
			if (_Time.y < crossfadeData.y)
			{
				...
				{
					const float1 lerpValue = (_Time.y - crossfadeData.x) / crossfadeData.z;
					normal = lerp(crossfadeNormalData, normal, lerpValue);	
				}
			}
		}
	}
